...
- Verification evidence is partial, not a full-suite pass. On 2026-08-17 both configured `poetry run` commands still failed before collection because the user-level Poetry launcher was not executable. In the dependency-complete repository `.venv`, the sixth-round candidate's equivalent raw `pytest tests/spec tests/contract` run recorded `609 passed / 6 failed / 4 errors`; the same run with a non-persistent workspace `--basetemp` recorded `613 passed / 6 failed`. The six failures already existed at baseline and comprise two root tests that hard-code an active waiver expiring on `2026-08-13`, followed by four fixture-cleanup cascades. The four raw-run errors are the Windows `%TEMP%/pytest-of-Administrator` ACL issue. Baseline `dfcb08fc` additionally had 22 control-contract failures; the sixth-round single-aggregate candidate has zero control-contract failures (`94 passed`). Those governance tests are outside TASK-022 allowed paths. Formal independent Review remains pending; `acceptance_status` is partial and no completion or release authorization is claimed.
...
- Current review-round evidence additionally covers the typed component ACK object and exact required-component set, the mandatory `CONTRACT-CONTROL-VALIDATION-CONTEXT-V1` authority/lineage DTO, shared Market RFC 8785 reference ordering/number vectors, authority-bound recovery barrier values, and full kill-switch command/result identity, fencing, authorization and UNKNOWN reconciliation semantics.
- The current remediation removes the prior public/configuration and kill-switch helper entrypoints; only `validate_control_message(message, validation_context)` is exposed, with private event dispatch and recovery-barrier validation invoked from that unified path. Barrier CLOSED, OPEN, and INVALIDATED states share authority/freshness validation before state-specific handling; stale/equal-freshness, future-observation, forged invalidation, and kill command/effect mutations are rejected fail-closed.
- Fifth-round remediation removes the remaining public `validate_dto` partial validator and proves the method/call graph with AST assertions. Config required-components and component capability facts are bound to a strict RFC 8785-checksummed authority context; kill-switch disable resolves a strict OPEN recovery-barrier registry record; and generic barrier `fresh_until` is schema-forbidden so only exact authority-bound `market_fresh_until` can establish freshness. Current-candidate command totals are recorded after final verification; Review remains pending.
- Sixth-round remediation replaces the flattened Kill Switch records in generic `identity_history` with one dedicated, bounded `kill_switch_registry`. A deterministic RFC 8785/SHA-256 command identity selects one append-once aggregate containing only an immutable semantically ACCEPTED command and at most one immutable result; result acceptance is a record-version 1 `result:null` to record-version 2 CAS. Exact lookup removes prefix interpretation and per-result registry scans, while schema plus aggregate checksum/fingerprint validation makes partial recovery fail closed. The 4096-entry cap is derived from the NFR workload and requires a verified durable audit checkpoint before compaction; saturation uses the existing `QQ-STORAGE-7001 IDEMPOTENCY_CONFLICT` semantics. Shared Message Envelope V1 is restored unchanged; canonical UTC-Z is a Control-owned combined-message refinement. Review remains pending.
