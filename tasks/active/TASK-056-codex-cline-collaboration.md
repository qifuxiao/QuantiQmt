...
协作协议，并让后续每个 active task 携带版本化、绑定精确 Base SHA 的 Codex
Implementation Plan。
...
- Observable outcome: Cline 从 GitHub 上的唯一 active task 获取 Codex Plan，基于其精确
  Base SHA 实现并提交 PR；新的 Codex Review 会话只读审查精确 Head；人类完成 merge
  和 closeout 授权。
...
- Authority chain、无 active、Base/dirty/plan gap fail-closed → Cline rule/adapter 文本测试。
...
- 禁止业务路径和规范变更 → exact changed-path audit 与现有 spec validator。

### Failure, concurrency, and recovery design

- Git Base 不等于 Plan Base、工作区有非预期修改、active task 数量不为一、task/spec 冲突、
  allowed paths 不足或设计缺口时，Cline 必须停止并返回 `PLAN_BLOCKED` 及证据。
...
3. 运行全部 verification commands 和 exact path audit。
...
- Planning base 已不再是实现分支祖先，或 active task/任务依赖状态发生变化。
...
- [ ] Task template 包含 Plan version、Planning base SHA、设计、文件计划、测试映射、失败设计
  和 PLAN_BLOCKED 条件。
- [ ] 没有修改 spec、业务代码、migration、依赖、CI、交易权限或现有任务 scope。
- [ ] 所有 verification commands 通过，独立 Review 后才可进入 closeout。

## Required evidence

- 使用 `ai/workflows/implement-task.md` 格式报告 changed files、逐项 acceptance、命令与
  exit code、未验证范围、风险和 spec deviations。
- Implementation Report 记录 exact Base/Head、branch/PR 和完整 allowed-path audit。
