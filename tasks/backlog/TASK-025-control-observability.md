...
implementation_contract:
  ...
  deliverables:
    ...
    - implement candidate ActiveVersion + config.version_activated.v1(CANDIDATE_COMMIT) + Outbox before component swap/ACK, and after complete rollback effects a strictly later restored ActiveVersion + config.version_activated.v1(ROLLBACK_RESTORE) + Outbox; public Events carry no component effects, while the internal APPLIED/REJECTED/PARTIAL/ROLLED_BACK/UNKNOWN result matrix binds the applicable Event references, safe-scope, stable identity and same-identity reconciliation
